...
const OUTPUT_DIR = path.resolve(__dirname, '../output/playwright/session-106-bl106-selfhosted-service-topology-final');
...
(async () => {
  ...
  // 2. Cluster web header (DEV/MOCK badges)
  ...
  // 3. Zammad setup/login page
  const zPage = await context.newPage();
  await zPage.goto(ZAMMAD_WEB);
  await zPage.waitForTimeout(3000);
  await screenshot(zPage, 'zammad-page-proof');
  ...
  const finalBoundary = [
    ...
    'Zammad topology: YES (Running, HTTP 200)',
