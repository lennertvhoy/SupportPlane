#!/usr/bin/env bash
# BL-121: Benchmark gemma4:e4b on user-local Ollama v0.22.0 (port 11435)
set -euo pipefail

BASE_URL="http://localhost:11435"
OUTPUT_DIR="output/playwright/session-111-bl121-local-model-runtime-upgrade"
mkdir -p "$OUTPUT_DIR"
...
if ! curl -sS --max-time 10 "$BASE_URL/api/tags" | grep -q "\"$MODEL\""; then
  echo "  Model $MODEL not found in Ollama tags."
...
RESP_FILE=$(mktemp)
START_MS=$(date +%s%3N)
HTTP_CODE=$(curl -sS -w "%{http_code}" --max-time 180 -X POST "$BASE_URL/api/generate" \
...
JSON_OUT="{\"model\":\"$MODEL\",\"loadSuccess\":$LOAD_SUCCESS,\"httpStatus\":\"$HTTP_STATUS\",\"latencyMs\":$LATENCY_MS,\"evalCount\":\"$EVAL_COUNT\",\"tokensPerSecond\":\"$TOKENS_PER_SEC\",\"jsonParseable\":$JSON_PARSEABLE,\"noWriteback\":$NO_WRITEBACK,\"noAutonomousSend\":$NO_AUTONOMOUS_SEND,\"noCloudClaim\":$NO_CLOUD_CLAIM,\"noSecret\":$NO_SECRET,\"fallbackUsed\":$FALLBACK_USED,\"error\":\"$ERROR_MSG\"}"
echo "$JSON_OUT" > "$OUTPUT_DIR/bl121-gemma4-benchmark.json"
python3 -m json.tool "$OUTPUT_DIR/bl121-gemma4-benchmark.json" > "$OUTPUT_DIR/bl121-gemma4-benchmark.txt" 2>/dev/null || cp "$OUTPUT_DIR/bl121-gemma4-benchmark.json" "$OUTPUT_DIR/bl121-gemma4-benchmark.txt"

echo "Benchmark complete."
echo "JSON: $OUTPUT_DIR/bl121-gemma4-benchmark.json"
echo "TXT:  $OUTPUT_DIR/bl121-gemma4-benchmark.txt"
