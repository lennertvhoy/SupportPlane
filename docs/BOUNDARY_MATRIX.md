...
| Kubernetes runtime | Cluster foundation + app/Postgres real | Kind/Podman cluster `supportplane-local` with API/Web/Worker in `supportplane-app` and PostgreSQL StatefulSet+PVC in `supportplane-data`. | Podman-backed local cluster with app/data/integration workloads. | Production cluster future only. | Cluster foundation + app/Postgres accepted. | n/a | Local placeholders only. | Local only. | Cluster commands, namespaces, Podman backing, image load, browser mock identity, pod restart survival. | No cloud or prod deployment claim. |
