...
| Kubernetes runtime | Cluster foundation real | Kind/Podman cluster `supportplane-local` and four namespaces exist; app services still run outside cluster. | Podman-backed local cluster with app/data/integration workloads. | Production cluster future only. | Cluster foundation only. | n/a | Local placeholders only. | Local only. | Cluster commands, namespaces, Podman backing, image load, browser mock identity. | No app-in-cluster, cloud, or prod deployment claim. |
