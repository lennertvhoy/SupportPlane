...
## Session 132 — BL-136 Proof Repair (CLOSURE-GRADE ACCEPTED)

**Date:** 2026-05-03 12:25 CEST
**Git HEAD:** to be recorded after final commit
**Branch:** main

### Why Repair Was Needed

Session 131 claimed BL-136 accepted but had four hard contradictions:
1. Runtime HEAD (e5b1dc) did not match final commit HEAD (94c961)
2. Git status evidence was captured pre-commit, not post-commit
3. AI policy mockOnly semantics were unreconciled (policy vs response)
4. Scenario C lacked actual RBAC denial proof (only admin dashboard screenshots)

### What Changed

1. **Rebuilt K8s images (API, Web, Worker):** Built fresh images from commit 94c961 codebase via `scripts/build_and_load_local_k8s_images.sh`. Deployed via `kubectl rollout restart`. Runtime HEAD now matches commit HEAD exactly.

2. **AI policy mockOnly reconciliation:** Investigated full code paths. Two different schemas share the same field name: `AiPolicy.safetyFlags.mockOnly` (admin guard — mock-as-safety-net cannot be disabled) vs `AiSafetyMetadata.mockOnly` (runtime transparency — this response was/wasn't generated by deterministic mock). `checkAiPolicy()` checks `allowedProviders` list (includes ollama), not `safetyFlags.mockOnly`. Both fields are correct in their respective contexts.

3. **RBAC denial proof:** Viewer identity confirmed (userRole=viewer). Viewer POST to `/support-sessions` returns 403 Forbidden: "support_session:create requires a higher role."

4. **Clean worktree proof:** git status captured after all commits show clean state.

5. **Full evidence re-capture:** 20 files (5 browser screenshots + 14 CLI artifacts + 1 index), all fresh, 0 duplicates.

### Verification

- Runtime HEAD: `94c961874070178442f819067d58d3665fcf3da3` — matches commit HEAD
- `npm run lint`: PASS (0 errors)
- `npm run typecheck --workspaces --if-present`: PASS (all workspaces)
- `python3 scripts/check_state_docs.py`: PASS
- `python3 scripts/check_docs_hygiene.py`: PASS
- `npm test --workspace=apps/api`: 210 pass, 0 fail, 3 skipped
- Git status: clean (`## main...origin/main [ahead 7]`)
- Evidence folder: `output/playwright/session-132-bl136-proof-repair/` (20 files)
- 5 unique PNG screenshots, 0 duplicates

### Evidence

- Folder: `output/playwright/session-132-bl136-proof-repair/`
- Files: 20 (exactly at cap)
- Screenshots: 15-cockpit-dashboard, 16-zammad-context, 17-ollama-draft (with mockOnly explanation), 18-governance-proof (audit+policy), 19-admin-dashboard
- CLI artifacts: health, git, pods, connectors, zammad detail, AI readiness, AI policy, ticket context, ollama draft, audit events, RBAC viewer denial, all policies, ollama models, zammad direct
- Index: 20-evidence-index.md (includes full mockOnly reconciliation)

### State File Updates

- STATUS.md: Updated timestamp, project state, evidence folder, API HEAD
- BACKLOG.md: BL-136 entry updated with Session 132 repair details
- PROJECT_STATE.yaml: Head entries, evidence folder, active queue updated
- WORKLOG.md: This entry appended
- docs/EVIDENCE_LOG.md: EV-2026-05-03-163 entry prepended
- docs/REALITY_MATRIX.md: Session number and key observations updated
- docs/ENTERPRISE_DEMO_GUIDE.md: Session number and scenario statuses updated

### Known Limitations (Honest)

- Windows endpoint (Scenario D) remains unverified — no real Windows host
- AI policy mockOnly safety default maintained (Ollama allowed via provider list; cloud AI blocked)
- OpenBao inmem storage (secrets lost on pod restart; requires re-seeding)
- API image rebuild required only because previous image predated final commit
- Full `npm test` across all workspaces not run (API only: 210 pass)
