...
- [BL-108] `[partial]` Ollama local AI provider integration. Added host-controlled local Ollama provider path for drafts with model name, prompt version, context hash, latency, no-cloud-call marker, no autonomous send marker, redaction before provider call, and deterministic labeled fallback. Runtime proof used deterministic fallback (`fallbackUsed=true`) rather than a successful host-controlled model call, so acceptance remains open. Dependencies: BL-106/BL-107. Non-claims: no cloud AI, no production AI governance, no autonomous send. Evidence: `output/playwright/session-109-bl108-109-110-115-real-sandbox-enablement/`.
