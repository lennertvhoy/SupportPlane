...
- [BL-108] `[accepted]` Ollama local AI provider integration. Host-controlled local Ollama provider path works from the cluster API via podman0 bridge (10.88.0.1:11434). Real model call proven with `fallbackUsed=false`, `provider=ollama`, `providerMode=local`, `model=llama3.1:8b`. Model selection benchmark attempted: gemma4 and qwen3.6 unavailable on Ollama 0.18.2; llama3.1:8b and qwen2.5:7b tested successfully. Model upgrade to gemma4/qwen3.6 remains future work requiring Ollama upgrade. Dependencies: BL-106/BL-107. Non-claims: no cloud AI, no production AI governance, no autonomous send. Evidence: `output/playwright/session-110-bl108-ollama-host-call-model-selection/`.
...
- [BL-121] `[planned]` Ollama model selection upgrade. After Ollama version upgrade, benchmark and select from gemma4:e4b, gemma4:26b, qwen3.6:27b, and other GPU-suitable models. Replace or supplement llama3.1:8b baseline. Dependencies: BL-108. Non-claims: no production AI governance. Evidence expected: benchmark table, real cluster call proof, UI metadata.
