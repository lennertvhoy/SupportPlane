...
- [BL-099] `[planned]` Connector Runtime Test Coverage + Documentation Hardening. Review BL-098 for gaps, add unit tests for `ConnectorRuntimeService` edge cases (null configs, boundary values, tenant isolation), update architecture docs with runtime flow diagrams. No new runtime behavior.
- [BL-100] `[planned]` Real Writeback Path Design Document. Draft architecture for real Zammad writeback with credential broker integration, approval gates, audit trail, and phased rollout plan. No implementation.
