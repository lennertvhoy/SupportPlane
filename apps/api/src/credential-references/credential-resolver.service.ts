...
export class CredentialResolverService {
  ...
  async resolveZammadApiToken(
    ...
    if (!enabled) {
      throw this.error('disabled', tenantId, credentialReferenceId, 'OpenBao sandbox resolver is disabled.');
    }

    if (!credentialReferenceId || credentialReferenceId === 'none') {
      throw this.error('missing', tenantId, credentialReferenceId, 'No credential reference is linked to this connector installation.');
    }

    const credential = await this.store.getCredentialReference(tenantId, credentialReferenceId);
    if (!credential) {
      throw this.error('missing', tenantId, credentialReferenceId, 'Credential reference is not currently locatable for this tenant.');
    }

    const secretRef = credential.secretRef;
    if (!secretRef || secretRef === 'local-dev-placeholder' || secretRef === '[REDACTED]') {
      throw this.error('missing', tenantId, credential.id, 'Credential reference has no OpenBao sandbox path.');
    }

    const addr = env('OPENBAO_ADDR');
    const token = env('OPENBAO_TOKEN');
    if (!addr || !token) {
      throw this.error('disabled', tenantId, credential.id, 'OpenBao address/token is not configured for the API runtime.');
    }

    const apiToken = await this.fetchOpenBaoSecret(addr, token, secretRef);
    if (!apiToken) {
      throw this.error('missing', tenantId, credential.id, 'OpenBao secret did not contain a Zammad API token.');
    }
  ...
  async getResolutionMetadata(
    ...
    return {
      ...
      secretPath: credential.secretRef && credential.secretRef !== '[REDACTED]' ? this.safePathLabel(credential.secretRef) : undefined,
  ...
  private async fetchOpenBaoSecret(addr: string, token: string, secretRef: string): Promise<string | undefined> {
  ...
  private error(status: CredentialResolutionMetadata['status'], tenantId: string, credentialReferenceId: string, message: string): Error & { metadata: CredentialResolutionMetadata } {
