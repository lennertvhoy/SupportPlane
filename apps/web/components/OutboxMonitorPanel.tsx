...
export function OutboxMonitorPanel({
  identity,
  onChanged,
}: {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setWorker(status);
      const selectedStillExists = selectedRef.current
        ? outbox.outboxItems.find((item) => item.id === selectedRef.current?.id)
        : undefined;
  ...
  const mayMutate =
    can(identity, 'outbox:process_once') ||
    can(identity, 'outbox:retry') ||
    can(identity, 'outbox:dead_letter') ||
    can(identity, 'outbox:cancel');
      ...
      <div className="space-y-3">
        ...
        {worker && (
          <div className="rounded border border-cockpit-700 bg-cockpit-900/40 p-2 text-[11px] text-cockpit-300">
            ...
            <div>
              Durable local worker · fallback:{' '}
              {worker?.fallbackQueueBackend ?? 'postgres-local-outbox'} ·{' '}
              {worker?.writebackEnabled ? 'sandbox writeback enabled' : 'writeback blocked'}
            </div>
          </div>
        )}
        ...
        {!mayMutate && (
          <div className="rounded border border-cockpit-700 px-2 py-1 text-[11px] text-cockpit-400">
            Read-only role. Inspecting is allowed; retry, process, cancel, and dead-letter controls
            are server-restricted.
          </div>
        )}
        {error && (
        ...
        <div className="space-y-1">
          {items.map((item) => (
            ...
            >
              <Badge
                variant={
                  item.status === 'mock_delivered' || item.status === 'sandbox_delivered'
                    ? 'success'
            ...
            <div className="mb-2 flex items-center justify-between gap-2">
              <div>
                <div className="font-medium text-cockpit-100">
                  Selected outbox {short(selected.id)} / {selected.status}
                </div>
                <div>
                  Action {short(selected.supportActionId)} / idempotency {selected.idempotencyKey}
                </div>
                <div>
                  nextAttemptAt {selected.nextAttemptAt ?? 'not scheduled'} / deadLetterReason{' '}
                  {selected.deadLetterReason ?? 'none'}
                </div>
                <div>
                  mode: {selected.deliveryMode ?? 'mock'} / realNetwork:{' '}
                  {String(latestDeliveryFlag(attempts, selected, 'realNetwork'))} /
                  writebackEnabled:{' '}
                  {String(latestDeliveryFlag(attempts, selected, 'writebackEnabled'))} /
                  externalWriteAttempted:{' '}
                  {String(latestDeliveryFlag(attempts, selected, 'externalWriteAttempted'))}
                </div>
              </div>
            </div>
            <div className="mb-2 flex flex-wrap gap-2">
              <button
                disabled={
                  !can(identity, 'outbox:process_once') ||
                  busy !== null ||
                  !['queued', 'retry_scheduled'].includes(selected.status)
                }
                onClick={() => run('process', () => api.processOutboxOnce(selected.id))}
                className="inline-flex items-center gap-1 rounded border border-cockpit-600 px-2 py-1 text-xs text-cockpit-200 disabled:opacity-50"
              >
                <Play size={12} /> Process once
              </button>
              <button
                disabled={
                  !can(identity, 'outbox:retry') ||
                  busy !== null ||
                  !['failed', 'retry_scheduled', 'dead_lettered'].includes(selected.status)
                }
                onClick={() => run('retry', () => api.retryOutboxItem(selected.id))}
                className="inline-flex items-center gap-1 rounded border border-cockpit-600 px-2 py-1 text-xs text-cockpit-200 disabled:opacity-50"
              >
              ...
              <button
                disabled={
                  !can(identity, 'outbox:dead_letter') ||
                  busy !== null ||
                  !['failed', 'retry_scheduled', 'processing'].includes(selected.status)
                }
                onClick={() =>
                  run('dead-letter', () =>
                    api.deadLetterOutboxItem(selected.id, 'Admin forced local mock dead-letter'),
                  )
                }
                className="inline-flex items-center gap-1 rounded border border-red-700/50 px-2 py-1 text-xs text-red-300 disabled:opacity-50"
              >
                <Skull size={12} /> Dead-letter
              </button>
              <button
                disabled={
                  !can(identity, 'outbox:cancel') ||
                  busy !== null ||
                  !['queued', 'processing', 'failed', 'retry_scheduled'].includes(selected.status)
                }
                onClick={() =>
                  run('cancel', () =>
                    api.cancelOutboxItem(selected.id, 'Admin cancelled local mock outbox item'),
                  )
                }
                className="inline-flex items-center gap-1 rounded border border-amber-700/50 px-2 py-1 text-xs text-amber-300 disabled:opacity-50"
              >
            ...
            <div className="space-y-1">
              ...
              {attempts.map((attempt) => (
                <div key={attempt.id} className="rounded border border-cockpit-700 px-2 py-1">
                  ...
                  {attempt.errorMessage ?? 'no error'} / realNetwork:{' '}
                  {String(
                    (attempt.deliveryResult as Record<string, unknown>)?.realNetwork ?? false,
                  )}{' '}
                  / writeback:{' '}
                  {String(
                    (attempt.deliveryResult as Record<string, unknown>)?.writebackEnabled ?? false,
                  )}{' '}
                  / externalWriteAttempted:{' '}
                  {String(
