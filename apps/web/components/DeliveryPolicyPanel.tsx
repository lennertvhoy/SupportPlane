...
export function DeliveryPolicyPanel({ identity }: { identity: AuthIdentity }) {
  ...
  const [validation, setValidation] = useState<{ policy: DeliveryPolicy; decision: DeliveryPolicyDecision } | null>(null);
          ...
          <div className="space-y-2">
            <div className="flex flex-wrap gap-1">
              {policies.map((p) => (
                <button
                  key={p.id}
                  onClick={() => { setSelectedPolicy(p); setValidation(null); setReadiness(null); }}
            ...
            {selectedPolicy && (
              <div className="space-y-3 rounded border border-cockpit-700 bg-cockpit-800/40 p-3">
                <div className="flex items-center justify-between">
                  <div className="text-xs font-semibold text-cockpit-100">{selectedPolicy.name}</div>
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-[10px] text-cockpit-500">v{selectedPolicy.policyVersion}</span>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      disabled={!canWrite || saving}
                      onClick={() => toggleSwitch('approvalRequired', !selectedPolicy.approvalRequired)}
                      className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <select
                      ...
                      onChange={(e) => handleUpdate({ minimumApproverRole: e.target.value as DeliveryPolicy['minimumApproverRole'] })}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-cockpit-400">{selectedPolicy.allowedActionTypes.join(', ')}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <input
                      ...
                      onChange={(e) => handleUpdate({ retryPolicy: { ...selectedPolicy.retryPolicy, maxAttempts: Number(e.target.value) } })}
                ...
                {validation && (
                  <div className={`rounded border p-2 text-[10px] ${
                    ...
                    <div className="flex items-center gap-1 font-semibold">
                      {validation.decision.allowed ? <CheckCircle size={10} /> : <XCircle size={10} />}
                    ...
                    <div className="mt-1 text-cockpit-500">
                      Mode: {validation.decision.mode} • Version: {validation.decision.policyVersion}
                    </div>
                  </div>
                )}
