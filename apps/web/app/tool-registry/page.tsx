...
function riskBadge(riskLevel: string) {
  ...
  if (riskLevel === 'low_risk_remediation') return <Badge variant="warning">Low-risk remediation</Badge>;
  if (riskLevel === 'elevated_remediation') return <Badge variant="danger">Elevated remediation</Badge>;
      ...
      <header className="flex items-center justify-between border-b border-cockpit-800 px-5 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-cockpit-500">Signed/local-verified fixed implementation tools only.</p>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-5xl space-y-4">
          <div className="rounded border border-cockpit-700 bg-cockpit-900/50 px-4 py-3 text-sm text-cockpit-300">
            <div className="flex items-center gap-2 font-medium text-cockpit-100">
              ...
              Manifests declare fixed implementation IDs only. They do not contain executable scripts.
            </div>
            <div className="mt-1 text-xs text-cockpit-500">
              No arbitrary shell · No argv injection · No user-provided command body · Fixed implementations verified by integrity hash
            </div>
          </div>
          ...
          <div className="rounded border border-cockpit-700 bg-cockpit-900/50 p-4">
            ...
            {tools.length === 0 ? (
              <div className="text-sm text-cockpit-500">No tools loaded. The local manifest may still be loading.</div>
            ) : (
              <div className="space-y-3">
                {tools.map((tool) => (
                  <div key={tool.id} className={`rounded border px-4 py-3 ${tool.enabled ? 'border-cockpit-700 bg-cockpit-950/50' : 'border-cockpit-800 bg-cockpit-950/30 opacity-60'}`}>
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2">
                        <Wrench size={15} className="text-cockpit-400" />
                        <span className="text-sm font-medium text-cockpit-100">{tool.displayName}</span>
                        <span className="text-xs text-cockpit-500">{tool.toolKey}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        {riskBadge(tool.riskLevel)}
                        {tool.approvalRequired && <Badge variant="warning">Approval required</Badge>}
                        {tool.enabled ? <Badge variant="success">Enabled</Badge> : <Badge variant="muted">Disabled</Badge>}
                      </div>
                    </div>
...
export default function ToolRegistryPage() {
  return <AuthGate>{(identity, logout) => <ToolRegistryContent identity={identity} logout={logout} />}</AuthGate>;
}
