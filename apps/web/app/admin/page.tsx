...
const cards = [
  ...
  { label: 'Model Usage', href: '/admin/model-usage', icon: BarChart3, color: 'border-cockpit-600' },
...
function AdminPageContent({ identity, logout }: { identity: AuthIdentity; logout: () => Promise<void> }) {
  const router = useRouter();
  const isAdmin = identity.permissions.includes('*') || identity.roles.includes('admin') || identity.roles.includes('owner');

  return (
    <AdminDashboardShell identity={identity} logout={logout} title="Admin Dashboard" subtitle="Control plane for policies, users, and governance.">
...
export default function AdminPage() {
  return <AuthGate>{(identity, logout) => <AdminPageContent identity={identity} logout={logout} />}</AuthGate>;
}
